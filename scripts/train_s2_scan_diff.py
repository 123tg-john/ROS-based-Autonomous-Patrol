...
"""
================================================================================================
ROS 自主巡邏機器人 - 深度強化學習訓練框架 (第三階段)
================================================================================================
本腳本是整個專案的核心訓練程序，負責以下關鍵任務：
1.  **初始化**: 建立 ROS 節點，並與 Gazebo 模擬環境 (`TurtleBot3EnvStage3`) 進行連接。
2.  **模型定義**: 定義了 SAC 演算法所需的神經網路結構，包括客製化的策略網路 (Policy) 和 Q網路 (Critic)。
3.  **智慧啟動**: 實現了複雜的啟動邏輯，可選擇從頭開始進行「遷移學習」、從指定的檢查點「強制恢復」，或自動從最新的進度「斷點續訓」。
4.  **進階訓練策略**: 整合了「學習率排程」等技術來優化和穩定訓練過程。
5.  **全方位監控**: 使用 TensorBoard 記錄詳細的內部指標，並透過 ROS Topic 發佈高層次的 KPI 指標。
6.  **穩健性設計**: 包含安全的終止保存機制，確保長時間訓練的穩定與可靠。
"""
...
class PolicyNetworkStage3(nn.Module):
    ...
    def __init__(self, state_dim, action_dim, hidden_dim=256):
        ...
        # ★★★ 最終優化：實現「公平」的初始判斷，讓異常分數的初始輸出接近 0.5 ★★★
